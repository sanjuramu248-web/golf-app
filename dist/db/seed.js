...
async function seed() {
  ...
  `);
    ...
    const inserted = await db_1.db.insert(schema_1.charities).values(charitySeed.map((c) => ({ ...c, image: null }))).onConflictDoNothing().returning({ id: schema_1.charities.id, name: schema_1.charities.name });
    ...
    if (inserted.length > 0) {
        ...
        const events = [];
        if (golfForGood) {
            events.push({ charityId: golfForGood.id, title: "Summer Charity Golf Day", description: "Annual 18-hole charity scramble at Wentworth. All proceeds to youth programmes.", eventDate: new Date("2026-07-15") }, { charityId: golfForGood.id, title: "Junior Golf Camp", description: "Free golf coaching for kids aged 8–16 from disadvantaged backgrounds.", eventDate: new Date("2026-08-10") });
        }
        if (fairway) {
            events.push({ charityId: fairway.id, title: "Mental Health Awareness Round", description: "9-hole social round with mindfulness sessions. Open to all skill levels.", eventDate: new Date("2026-06-20") });
        }
        if (events.length > 0) {
            await db_1.db.insert(schema_1.charityEvents).values(events).onConflictDoNothing();
